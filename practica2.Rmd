...
library(tidyverse)
library(ggplot2)
a<-ggplot(ds, aes(x =MinTemp, y = MaxTemp))
a

a1<-a+geom_point()
a1

a2<-a+geom_point(aes(color=RainToday))
a2


a3<-a2+ggtitle('Relación entre temperaturas máxima y mínima y probabilidad de lluvia')
a3

#scale_color_manual(values=c(azul,rosa)) para cambiarles el color
...
b<-ggplot(ds, aes(x =Temp9am, y = Pressure9am))
b

b1<-ggplot(ds, aes(x =Temp9am, y = Pressure9am,shape=RainToday))+geom_point()
b1

b2<-ggplot(ds, aes(x =Temp9am, y = Pressure9am,color=RainToday))+geom_point()
b2

b3<-ggplot(ds, aes(x =Temp9am, y = Pressure9am,color=RainToday))+geom_point()+labs(tittle='Probabilidad de lluvia según condiciones meteorológicas',x='Temperatura 9AM(ºC)',y='Presión atmossférica 9AM(mb)')
b3
...
ggplot(iris.tidy, aes(x =Species, y = Value,color=Part))+facet_grid(.~Measure)+geom_jitter()
...
ggplot(DatosEjer4, aes(x=Fecha, y=Ventas, color=Fabricante)) + geom_point()+geom_line(aes(shape=Fabricante))

ggplot(DatosEjer4, aes(x=Fecha, y=Ventas, color=Fabricante)) + geom_point()+geom_line(aes(shape=Fabricante))+labs(title="Ventas mensuales de diferentes compañías tecnológicas")
...
ggplot(Experimento,aes(x=Dosis,y=Altura, shape=Complemento, linetype=Complemento))+geom_line()+geom_point()

ggplot(Experimento,aes(x=Dosis,y=Altura, shape=Complemento, linetype=Complemento))+geom_line()+geom_point()+geom_errorbar(aes(ymin = Altura - Error_Altura,
                    ymax = Altura + Error_Altura),
                width = 0.1,
                position = position_dodge(0.05))

ggplot(Experimento,aes(x=Dosis,y=Altura))+geom_line(aes(linetype=Complemento))+geom_point(aes(shape=Complemento))+geom_errorbar(aes(ymin = Altura - Error_Altura,
                    ymax = Altura + Error_Altura),
                width = 0.1,
                position = position_dodge(0.05))+scale_color_brewer(palette="Reds")
...
ggplot(ice.river.df,aes(x=time))+geom_line(aes(y=flow.vat),color='red')+geom_line(aes(y=flow.jok),color='blue')+labs(title='Caudales de los ríos islandeses Vatnsdalsa (azul) y Jokulsa Eystri (rojo)',x='Tiempo (años)', y='Caudal (m^3/s)')
...
ggplot(ds,aes(x=Year,y=MinTemp))+geom_boxplot()
ggplot(ds,aes(x=Year,y=MaxTemp))+geom_violin(aes(fill = Year))+labs(title='Distribución de temperaturas máximas por año',x='Año',y='Temperatura (ºC)')+theme(
